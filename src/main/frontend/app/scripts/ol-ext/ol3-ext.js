...
{	var options = opt_options || {};
	...
	var element;
	if (options.target) 
	{	element = $("<div>").addClass(options.switcherClass || "ol-layerswitcher layer-switcher");
	}
	else
	{	element = $("<div>").addClass((options.switcherClass || 'ol-layerswitcher layer-switcher') +' ol-unselectable ol-control ol-collapsed');
		...
		this.button = $("<span>")
					.attr('title',tipLabel)
					.attr('class','fab-layer toggle-layers-button')
					.html(b + c + d)
...
{   ol.control.Control.prototype.setMap.call(this, map);
	...
	{	map.getLayerGroup().on('change', this.drawPanel, this);
		...
		map.on('pointerdown', function(){
			if($('.ol-layerswitcher-image').hasClass("ol-forceopen")) {
				$('.ol-layerswitcher-image').toggleClass("ol-forceopen").addClass("ol-collapsed"); 
				self.overflow();
			}
		});
	}
...
{	
	...
	layers.forEach(function(li){	
		if (l!==li && li.get('type')==='base') li.setVisible(false);
	});	
...
{	if (this.get('maxResolution')<e.frameState.viewState.resolution) return;
	...
	ctx.save();
		...
		{	ctx.strokeStyle = this.style.getStroke().getColor();
			...
			ctx.setLineDash(this.style.getStroke().getLineDash());
			ctx.stroke();
		}
...
{	var self = this;	
	...
	var showLayerName = function(e) 
	{	e.preventDefault(); 
		if (e.type=="mouseover") $(this).find('p').addClass("show");
	};
	var hideLayerName = function(e) 
	{	e.preventDefault(); 
		if (e.type=="mouseleave") $(this).find('p').removeClass("show");
	};
	...
	{					
		if (layer.get("displayInLayerSwitcher")!==false){	
			...
			var prev = layer.getPreview ? layer.getPreview(center,2000) : ["none"];
			var d = $("<li>").addClass("ol-imgcontainer").addClass("shadowed")
						.data ('layer', layer)
						.click (setVisibility)
						.on ("touchstart", setVisibility)
						.on ("mouseover", showLayerName)
						.on ("mouseleave", hideLayerName);
			if (layer.getVisible()) d.addClass("select");
			for (var k=0; k<prev.length; k++)
			{	$("<img>").attr('src', prev[k]).attr('draggable', 'false')
						.appendTo(d);
			}
	...
	$('.ol-imgcontainer > img').error(function(){
		$(this).hide();
	});
};
...
function resizeLayerSwitcher() {
	var TOP_HEIGHT=60;
	setTimeout(function(){
		if($('.ol-forceopen')) {
			if(($('.ol-unselectable').height() - TOP_HEIGHT)<= $('.ol-forceopen').height()) {

				$('.ol-forceopen > ul').css("height", ($('.ol-unselectable').height() - TOP_HEIGHT)+"px")
			} else {
				$('.ol-forceopen > ul').css("height", "100%");
				$('.ol-forceopen > ul').css("height", "auto");
			} 
		}
	},0);
